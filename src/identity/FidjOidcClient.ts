...
export class FidjOidcClient {
    ...
    constructor(private options: OidcOptions) {
        const issuer = new URL(options.issuer),
            api = new URL(options.apiEndpoint),
            redirect = new URL(options.redirectUri);
        if (
            issuer.origin !== api.origin ||
    ...
    private network() {
        return {
            ...(this.options.issuer.startsWith('http:')
    ...
    private async discover(issuer: URL) {
        try {
            return await oauth.processDiscoveryResponse(
                issuer,
                await oauth.discoveryRequest(issuer, {...this.network(), algorithm: 'oidc'})
            );
        } catch (cause) {
            throw Object.assign(
                new Error(
                    'No OpenID Connect provider answered at ' +
                        issuer.href +
                        '. OIDC support is beta: check the deployment configures an issuer and signing keys before using it.'
                ),
                {cause}
            );
        }
    ...
    private async discovery() {
        if (!this.metadata) {
            ...
            this.metadata = await this.discover(issuer);
            ...
            if (
                ...
                new URL(String(this.metadata.end_session_endpoint)).origin !== issuer.origin
            ) {
                throw new Error('Unexpected identity endpoint origin');
            }
        }
    ...
    async beginLogin(options: {silent?: boolean; prompt?: string} = {}) {
        ...
        url.search = new URLSearchParams({
            ...
            response_type: 'code',
            scope: 'openid profile email offline_access fidj:api',
            ...(prompt ? {prompt} : {}),
            state,
            nonce,
    ...
    async completeLogin(callback: URL) {
        ...
        if (refusal) {
            throw Object.assign(
                new Error(callback.searchParams.get('error_description') || refusal),
                {
                    code: refusal,
                    silentRefusal: [
                        'login_required',
        ...
        const as = await this.discovery(),
            client = {client_id: this.options.clientId};
    ...
    private save(tokens: any, identity: any) {
    ...
    async request(path: string, method = 'GET', data?: unknown): Promise<any> {
        ...
        const response = await fetch(this.options.apiEndpoint.replace(/\/$/, '') + path, {
            method,
            headers: {
    ...
    private async endSessionUrl(): Promise<string | undefined> {
        const session = this.session(),
            endpoint = (await this.discovery()).end_session_endpoint;
        if (!endpoint || !session?.tokens?.id_token) {
            return undefined;
        }
    ...
    async logout(options: {endProviderSession?: boolean} = {}): Promise<string | undefined> {
        const endSession = options.endProviderSession
            ? await this.endSessionUrl().catch(() => undefined)
            : undefined;
        let confirmed = false;
        try {
            if (this.hasSession()) {
                await this.request('/me/oidc/logout', 'POST', {

...
export async function verifyAppSession(
    ...
    try {
        ...
        if (token.split('.').length !== 3) {
            ...
            payload = JSON.parse(
                Base64.decode(encoded.padEnd(Math.ceil(encoded.length / 4) * 4, '='))
            );
            if (
    ...
    if (
        !payload &&
        (typeof (result as any).subject !== 'string' || (result as any).appId !== options.appId)
    ) {
...
export async function verifyOrganizationSession(
    token: string,
    options: {appId: string; apiEndpoint: string; organizationId: string; permission?: string}
) {
    const session = await verifyAppSession(token, options);
    let response: Response;
    try {
        response = await fetch(
            `${options.apiEndpoint.replace(/\/$/, '')}/apps/${encodeURIComponent(options.appId)}/organizations/${encodeURIComponent(options.organizationId)}`,
            {
    ...
    if (
        result.organization?.id !== options.organizationId ||
        !Array.isArray(result.access?.permissions) ||
        (options.permission && !result.access.permissions.includes(options.permission))
    ) {
    ...
    return {
        ...
        permissions: result.access.permissions as string[],
    };
}
